...
from modules import (MLP, MLP_static, MLP_const, MLP_convex, MLP_convex_static, Biaffine, Biaffine_convex, BiLSTM, CHAR_LSTM, CHAR_CNN, CHAR_LSTM_D, SharedDropout, IndependentDropout)
...
class GlobalEnergyModel(nn.Module):
    ...
    def __init__(self, device, config, VOCAB_SIZE, CHAR_SIZE, POS_SIZE, DEPREL_SIZE, bert=None, embed=0, xembed=None):
        ...
        self.mlp_arc_d = MLP(n_in=config.hidden_size*2,n_out=config.arc_mlp_size,dropout=0.33)
        self.mlp_arc_h = MLP(n_in=config.hidden_size*2,n_out=config.arc_mlp_size,dropout=0.33)
        self.mlp_rel_d = MLP(n_in=config.hidden_size*2,n_out=config.rel_mlp_size,dropout=0.33)
        self.mlp_rel_h = MLP(n_in=config.hidden_size*2,n_out=config.rel_mlp_size,dropout=0.33)
    ...
    def LinearEnergyMatrix(self, features, batch_size, max_length=None):
        ...
        return self.LinearEnergy(Heads, Modifiers, batch_size, max_length), self.LinearRel(Heads, Modifiers, batch_size, max_length)
    ...
    def Initialization(self, sent, bsent, char, pos, deprel, tree, batch_size, lengths, mask, mask1, mask2):
        ...
        (self.E, self.linearMatrix), (self.logRel, self.Erel) = self.LinearEnergyMatrix(self.features, self.batch_size, self.max_length)
