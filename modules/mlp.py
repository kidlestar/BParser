...
from modules.dropout import SharedDropout, SharedDropout_convex
import torch
...
class MLP_static(nn.Module):
    ...
    def __init__(self, n_in, n_hidden, sflag=True):
        super(MLP_static, self).__init__()

        if sflag: self.linear = spectral_norm(nn.Linear(n_in, n_hidden))
        else: self.linear = nn.Linear(n_in, n_hidden)
        self.activation = nn.LeakyReLU(negative_slope=0.1)
    ...
    def evaluate(self,x,training=True):
        #print(x)
        #print(self.linear.weight.detach())
        #print(self.linear.bias.detach())
        ...
        #x=F.linear(x, self.linear.weight.detach(), self.linear.bias.detach())
        x=self.activation(x)
        return x
